...
function replace_with_longterm_mean!(tmp::Matrix, selected_columns::Vector{String}, actual_column_names::Vector{String},plot_flag::Bool)
    ...
    for col_name in selected_columns
        ...
        if col_idx === nothing
            ...
            if (col_name .== "SW_OUT") || (col_name .== "LW_OUT")
            else
...
enddates = driver_data_1[2:end, column_names.=="TIMESTAMP_START"]; # time yyyymmddhhmm
...
replace_with_longterm_mean!(driver_data, selected_columns,column_names, plot_input_var)#all missing and below -900 datasets are replaced by long-term weekly mean
...
LW_IN_QC = driver_data[2:end, column_names.=="LW_IN_F_QC"]
...
global SWC = driver_data[2:end, column_names.=="SWC_F_MDS_1"] ./ 100; # to convert from % to m^3/m^3
...
if spinup .== 1
    ...
    LAI_repeated = repeat(LAI_filtered, inner= Int64(round(length(seconds)./365)))
end
