...
cur_dir = @__DIR__
#cur_dir="/Users/mitraasadollahi/Projects/CliMA/ClimaLSM.jl/experiments/integrated/US-Ton"
include(
    joinpath(
        cur_dir,
        "load_packages.jl",
...
global start_year =3#minimum(daily)
global end_year = 3#maximum(daily)
global start_date = 1 + (start_year - 1) * 365#maximum(daily)
global end_date = 200 + (end_year - 1) * 365#maximum(daily)
...
# Define the path for the error log file in the same directory

global error_log_path = joinpath(cur_dir, "error_log.txt")
if isfile(error_log_path)
    rm(error_log_path)
end
#=try
    sol = SciMLBase.solve(
    ...
    # If an error occurs, write the error message and the stacktrace to the log file
    open(error_log_path, "w") do file
        write(file, "Error: $(err)\n")
        write(file, "Stacktrace:\n")
        write(file, sprint(Base.show_backtrace, catch_backtrace()))
    end
    #rethrow(e)  # If you want to propagate the error after logging
end
=#

#if !isdefined(Main,:sol) .& .!isfile(error_log_path)
    sol = SciMLBase.solve(
        prob,
        ode_algo;
        dt=dt,
